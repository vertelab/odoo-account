...
class AccountInvoice(models.Model):
    ...
    def _reverse_invoice(self, invoice_id, credit_invoice_ref, company_id):
        ...
        refund_invoice_wiz = self.env['account.invoice.refund'].with_context(wiz_context).create({
            'filter_refund': 'refund',
            'date': fields.Date.today(),
        })

        refund_invoice = self.env['account.invoice'].browse(refund_invoice_wiz.invoice_refund()['res_id'])
    ...
    def update_invoice_status_fortnox_cron(self):
        ...
        for company_id in self.env['res.company'].search([]):
            move_id = self.env['account.invoice'].search([
