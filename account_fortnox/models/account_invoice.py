    ...
    def fortnox_create(self):
        ...
        for invoice in self:
                ...
                InvoiceRows.append(
                    {
                        ...
                        # ~ "Unit": "st",
                        "VAT": int(line.invoice_line_tax_ids.mapped('amount')[0]) if len(
                            line.invoice_line_tax_ids) > 0 else None,
                    })
            ...
            if r.get('ErrorInformation'):
                invoice._message_log(body='Error Creating Invoice Fortnox %s ' % r['ErrorInformation']['message'],
