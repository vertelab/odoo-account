...
class MisBudgetAccount(models.Model):
    ...
    def action_create_income_budget(self):
        ...
        return \
        self.env.ref('account_mis_budget.action_make_account_budget').with_context({'res_id': wizard_id.id}).read()[0]
...
class AccountClass(models.Model):
    ...
    account_class = fields.Char(string="Account Class",
                                help="Coma separated list of beginning of account code for account classes eg 5,6")
                ...
                for account in self.env['account.account'].search([('code', 'like', f"{ac_begin}___"), ]):
                    if use_account_from_year:
                        nbr_use = self.env['account.move.line'].search_count(['&',
                                                                              ('date', '>=',
                                                                               fields.Date.today() - relativedelta.relativedelta(
                                                                                   years=1)),
...
class MakeAccountBudget(models.TransientModel):
    ...
    use_account_from_year = fields.Boolean(string="Use accounts from last year?",
                                           help="If checked in we create lines only for accounts that were used in an "
                                                "account.move.line last year",
                                           default=True)

    account_class_ids = fields.Many2many(comodel_name='custom.account.class', string='Account class')
    percentage_factor = fields.Float(string='Percentage Factor', default=1)

    def make_report(self):
        for account in self.account_ids:
            for date_range in self.budget_id.date_type.date_range_ids:
                if (date_range.date_start >= self.budget_id.date_from) and (
                        date_range.date_end <= self.budget_id.date_to):
                    ...
                    if not budget_item:  # Do not overwrite a current budget item
                        ...
                        if self.use_account_from_year:  # Use historic data for the budget
                            balance = sum(self.env['account.move.line'].search([
                                ...
                                ('date', '<=', date_range.date_end - relativedelta.relativedelta(years=1))]).mapped(
                                'balance'))
                            if balance > 0:
