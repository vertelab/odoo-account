...
class ResEnablebanking_Config(models.Model):
    ...
    def action_sync_transactions_with_enable_banking(self, bank_id):
        ...
        auth_url = self.auth_request(
            bank_id, api_url=api_url, base_headers=base_headers)
        return auth_url
    ...
    def _request_application_details(self, api_url, base_headers):
        ...
        if not base_headers:
            api_url, private_key, application_id, base_headers = self.request_essentials()
        application_resp = requests.get(
            f"{api_url}/application", headers=base_headers).json()
    ...
    def auth_request(self, bank_id=None, app=None, api_url=None, base_headers=None):
        ...
        _logger.warning(
            f"{bank_id.name=},{bank_id.country} {bank_id.country.code=}")
        body = {
            "access": {
        ...
        auth_resp = requests.post(
            f"{api_url}/auth", json=body, headers=base_headers).json()
        return auth_resp
