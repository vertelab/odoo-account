...
class account_invoice(models.Model):
    ...
    def upload_invoice(self, xml_string):
        url = "https://testapi.inexchange.se/v1/api/documents"
        for invoice in self:
            r = self.env['res.config.settings'].inexchange_invoice_upload(
              url, xml_string)
            return r
    ...
    def send_uploaded_invoice(self):
        # all infomation are in the log, but because of other problems,
        # it can not be upload to inexchange, I have to fix boundary
        # first and then get the location string back then this step
        # will be processed.
    ...
    def send_and_print_action(self):
        ...
        if self.is_inexchange:
            for invoice in self.invoice_ids:
                ...
                raise Warning(f'{result.status_code}\n {result.text}')
