...
class BankPaymentLine(models.Model):
    ...
    def create(self, vals):
        ...
        if res.currency_id.name != "SEK" and self.env.company.country_code == 'SE' and not res.regulatory_reporting_code and res.amount_company_currency >= 150000:
            error_args = {"line_partner": res.partner_id.name, "line_communication": res.communication,
                          "line_amount_sek": res.amount_company_currency, "line_amount_euro": res.amount_currency,
                          'line_currency': res.currency_id.name}
            raise UserError(_("Regulatory Reporting Code is missing in a line\n\n"
                              "{line_partner} {line_communication} {line_amount_euro}\n\n"
                              "Regulatory Reporting Code is required when the currency is other then SEK ({"
                              "line_currency}) and when the value ({line_amount_sek}) is greater or equal to 150000 "
                              "SEK\n\n"
