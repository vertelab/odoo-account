...
class AccountPeriod(models.Model):
    ...
    def find(self, dt=None, context=None):
        ...
        args = [('date_start', '<=', dt), ('date_stop', '>=', dt), ('company_id', '=',
            self.env.context.get('company_id', self.env['res.company']._company_default_get('account.account')))]
        result = []
    ...
    def action_draft(self):
        ...
        self.env.cr.execute('update account_journal_period set state=%s where period_id in %s',
                            (mode, tuple(self.mapped('id')),))
...
class account_account(models.Model):
    ...
    def sum_period(self):
        ...
        domain = [('move_id.period_id', 'in',
                   ...
                   self._context.get('period_stop', self._context.get('period_start')))),
                  ('account_id', '=', self.id)]
        _logger.warn('\n\n\n\n\n domain: %s' % domain)
