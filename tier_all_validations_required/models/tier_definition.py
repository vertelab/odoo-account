...
class TierDefinition(models.Model):
    ...
    reviewers_validations_required = fields.Boolean(default=False,
                                                    string="All reviewers validations required")  ### attesterare
...
class TierValidation(models.AbstractModel):
    ...
    def _validate_tier(self, tiers=False):
        ...
        for review in multi_reviews:
            if review.reviewer_ids and review.done_by_all and sorted(review.reviewer_ids.ids) == sorted(
                    review.done_by_all.ids):
...
class TierReview(models.Model):
    ...
    status = fields.Selection(selection_add=[("partial_approved", "Partially approved")],
                              ondelete={'partial_approved': 'set default'})

    done_by_all = fields.Many2many(comodel_name="res.users", relation="done_validations")
    reviewers_validations_required = fields.Boolean(related='definition_id.reviewers_validations_required',
                                                    readonly=True)
