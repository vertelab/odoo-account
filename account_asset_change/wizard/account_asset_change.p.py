...
class AssetModify(models.TransientModel):
    ...
    def _get_selection_modify_options(self):
        ...
        return [
            ...
            ('pause', _("Pause")),
            ('move', _("Move Analytic")),
        ]

    @api.depends('company_id')
    def _compute_accounts(self):
        for record in self:
            record.gain_account_id = record.asset_id.profile_id.account_plus_value_id
            record.loss_account_id = record.asset_id.profile_id.account_min_value_id
        ...
        for wizard in self:
            if wizard.modify_action == 'dispose':
                ...
                wizard.informational_text = _(
                    ...
                    "Future entries will be recomputed to use the new analytic account distribution",
                    date=format_date(self.env, wizard.date), extra_text=text,
                )
            else:
                if wizard.gain_value:
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if 'asset_id' in vals:
                ...
                if asset.depreciation_move_ids.filtered(lambda m: m.parent_state == 'posted' and not m.move_id.reversal_move_ids and m.move_id.date > fields.Date.today()):
    ...
    def modify(self):
        ...
        if self.currency_id.compare_amounts(increase, 0) < 0:
            move = self.env['account.move'].create(self.env['account.move']._prepare_move_for_asset_depreciation({
                ...
                # ~ 'asset_move_type': 'negative_revaluation',  TODO
            }))._post()
        ...
        # ~ if computation_children_changed:   We dont have children TODO
            ...
            # ~ for child in children:
                ...
                # ~ child.depreciation_move_ids.filtered(lambda move: move.parent_state != 'posted')._post()
        ...
        self.asset_id.depreciation_move_ids.filtered(lambda move: move.parent_state != 'posted')._post()
