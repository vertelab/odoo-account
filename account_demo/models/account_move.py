...
class AccountMove(models.Model):
    ...
    def _account_demo_load_moves(self):
        ...
        def resolve_account(ref_str):
            """Resolve account: try XML ID → code lookup → type fallback."""
            if not ref_str:
                return False
            # Try XML ID first
            aid = IrModelData._xmlid_to_res_id(ref_str)
            if aid:
                return aid
            # Extract code from XML ID name (e.g. account_demo.aa_1579 → code 3001)
            name_part = ref_str.rsplit('.', 1)[-1]
            # Try our code map
            for code_suffix, (code, atype) in demo_code_map.items():
                cache_key = (code, atype)
                if cache_key in account_cache:
                    continue
                account = self.env['account.account'].search([
                    ('code_store', '=like', f'%"{code}"%'),
                    ('company_id', '=', self.env.company.id),
                ], limit=1)
                if account:
                    account_cache[cache_key] = account.id
            # Resolve by looking at which demo account code matches
            # We need the JSON's account reference → code
            # For now, try all cached codes
            for (code, atype), acc_id in account_cache.items():
                pass  # Need to match ref_str to a specific code

            return False

        # Actually, the JSON references specific XML IDs. Let's resolve by
        # mapping aa_XXXX → account code → l10n_se account
        # The JSON uses: account_demo.aa_1523 → was account 2610
        # We map the XML ID to a code, then find l10n_se account by code
        xmlid_to_code = {
            'account_demo.aa_1523': '2610', 'account_demo.aa_1531': '2620',
            'account_demo.aa_1547': '2640', 'account_demo.aa_1562': '1510',
            'account_demo.aa_1567': '1930', 'account_demo.aa_1568': '2440',
            'account_demo.aa_1570': '3740', 'account_demo.aa_1573': '8990',
            'account_demo.aa_1579': '3001', 'account_demo.aa_1580': '3002',
            'account_demo.aa_1582': '4000', 'account_demo.aa_1583': '4001',
            'account_demo.aa_1584': '4002', 'account_demo.aa_1604': '6540',
            'account_demo.aa_1607': '6550', 'account_demo.aa_1608': '6560',
            'account_demo.aa_1609': '6570', 'account_demo.aa_1661': '1229',
            'account_demo.aa_1691': '1311', 'account_demo.aa_1813': '1710',
            'account_demo.aa_1815': '1730', 'account_demo.aa_1833': '1940',
            'account_demo.aa_1834': '1950', 'account_demo.aa_1887': '2126',
            'account_demo.aa_1957': '2420', 'account_demo.aa_2008': '2821',
            'account_demo.aa_2161': '4600', 'account_demo.aa_2186': '5010',
            'account_demo.aa_2187': '5011', 'account_demo.aa_2194': '5060',
            'account_demo.aa_2195': '5061', 'account_demo.aa_2200': '5070',
            'account_demo.aa_2211': '5160', 'account_demo.aa_2246': '5410',
            'account_demo.aa_2249': '5420', 'account_demo.aa_2302': '6000',
            'account_demo.aa_2315': '6070', 'account_demo.aa_2317': '6072',
            'account_demo.aa_2321': '6110', 'account_demo.aa_2324': '6210',
            'account_demo.aa_2326': '6212', 'account_demo.aa_2330': '6230',
            'account_demo.aa_2331': '6250', 'account_demo.aa_2333': '6310',
            'account_demo.aa_2370': '6970', 'account_demo.aa_2374': '6990',
            'account_demo.aa_2375': '6991', 'account_demo.aa_2376': '6992',
            'account_demo.aa_2470': '7410', 'account_demo.aa_2471': '7411',
            'account_demo.aa_2484': '7490', 'account_demo.aa_2507': '7583',
            'account_demo.aa_2510': '7600', 'account_demo.aa_2511': '7610',
            'account_demo.aa_2512': '7620', 'account_demo.aa_2517': '7631',
            'account_demo.aa_2518': '7632', 'account_demo.aa_2523': '7690',
            'account_demo.aa_2527': '7699', 'account_demo.aa_2551': '7832',
            'account_demo.aa_2654': '8400', 'account_demo.aa_2663': '8420',
            'account_demo.aa_2665': '8422', 'account_demo.aa_2668': '8429',
            'account_demo.aa_2706': '1930',   # Banktillgodohavande → 1930
            'account_demo.aa_2715': '1930',   # Bank → 1930 (fallback)
            'account_demo.aa_2716': '1930',   # Bankobservationskonto → 1930 (fallback)
            'account_demo.aa_3728': '4009',   # Pant
            'account_demo.aa_3729': '1930',   # Bankgiro → 1930
            'account_demo.aa_3730': '1930',   # Bankgiro → 1930
        }
        ...
        def resolve_account(ref_str):
            ...
            if row:
                return row[0]
            # Fallback: use any account with matching type
            _logger.warning('account_demo: account code %s not found, using fallback', code)
            self.env.cr.execute(
                "SELECT id FROM account_account WHERE account_type = 'asset_cash' LIMIT 1")
            row = self.env.cr.fetchone()
        ...
        def resolve_tax(ref_str):
            ...
            tax_map = {
                'account_demo.tax_71': ('MP1', 25.0, 'sale'),
                'account_demo.tax_72': ('I', 25.0, 'purchase'),
                'account_demo.tax_74': ('MP2', 12.0, 'sale'),
                'account_demo.tax_115': ('IT', 25.0, 'purchase'),
                'account_demo.tax_116': ('Ii', 25.0, 'purchase'),
                'account_demo.tax_117': ('I12', 12.0, 'purchase'),
                'account_demo.tax_118': ('IT12', 12.0, 'purchase'),
            }
