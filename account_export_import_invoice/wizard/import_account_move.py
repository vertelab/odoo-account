...
class ImportAccountMove(models.Model):
    ...
    account_move_file = fields.Binary(
        string="Account File (XML) ", attachment=True
    )
    account_move_filename = fields.Char(string="Account Filename")
    ...
    def action_import_account_move(self):
        xml_content = etree.fromstring(base64.b64decode(self.with_context(bin_size=False).account_move_file))
        ...
        line_tax_ids = dict_content.get('invoice_line_ids')
        tax_ids = [(4, self._tax_rec(tx[-1]).id) for tx in dict_content.pop('tax_ids')]
        #         create_vals = {
        #             **config,
        ...
        self.env['account.move'].create(dict_content)
